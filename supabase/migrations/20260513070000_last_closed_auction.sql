-- Add `last_closed_auction` to get_room_view during drafting. The UI uses
-- this to show an "Adjudicat" celebration banner when an auction transitions
-- from active to closed/skipped/auto_assigned.
...
begin
	...
	if v_room.status = 'drafting' then
		...
		-- The most recent resolved auction. Drives the "Adjudicat" banner.
		select jsonb_build_object(
			'id', a.id,
			'sequence_number', a.sequence_number,
			'status', a.status,
			...
			'player_id', a.player_id,
			'player_name', pl.name,
			'player_photo_url', pl.photo_url,
			'player_team', pl.metadata ->> 'team',
			'ends_at', a.ends_at
		)
		into v_last_closed
		from public.auctions a
		...
		where a.room_id = v_room.id
			and a.status in ('closed', 'auto_assigned', 'skipped')
	...
	return jsonb_build_object(
		...
		'last_closed_auction', v_last_closed,
		'teams', v_teams,
		'tally', v_tally
	);
end;
