-- Refine the bid timer extension: only extend by +5s when the remaining time
-- is under 10 seconds. Bidding with more than 10s left does NOT push the
-- timer back — keeps auctions snappy and prevents one team from stalling.
...
begin
	...
	set current_bid_cents = p_amount_cents,
		...
		ends_at = case
			when ends_at - now() < interval '10 seconds' then ends_at + interval '5 seconds'
			else ends_at
		end
